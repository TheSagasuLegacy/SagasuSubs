...
class UploadFiles:
    def __init__(self, db_path: Path, base: str):
    ...
    async def upload_file(self, file: dto.FileRead) -> models.FileRead:
        ...
        model = models.FileCreate(
            filename=file.filename, sha1=file.sha1, series=file.series_id
        )
    ...
    async def upload_dialogs(
        self, file_id: str, dialogs: List[dto.DialogRead]
        ...
        response = await self.client.post(
            "/api/dialogs/bulk", timeout=30, json={"bulk": bulk_data}
        )
        response.raise_for_status()
        logger.debug(f"Dialog for {file_id=} data sync to upstream.")
        return [*map(models.DialogRead.parse_obj, response.json())]
